// 신축이음 별도 처리를 위한 수정 스크립트

// 기존 generateEvaluationTable 함수를 오버라이드
function generateEvaluationTable_Original(componentType, data) {
    ...
    if (componentType === 'expansionJoint') {
        ...
        if (expansionLocations.length > 0) {
            ...
            // 신축이음 위치 정보가 없으면 기본 1행 생성
            console.log('신축이음 위치 정보 없음, 기본 행 생성');
            tableHtml += generateEmptyTableRow(componentType, '-', defaultInspectionArea);
        }
    }
